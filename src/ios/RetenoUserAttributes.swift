...
public struct RetenoUserAttributes {
    ...
    public static func buildAnonymousUserAttributesPayload (payload: NSDictionary) throws -> AnonymousUserAttributes{
        let user = payload["user"] as? [String: Any];
        do {
            let userStruct = try RetenoUserStruct(dictionary: user);
            let userAttributes = userStruct.userAttributes != nil ? buildAnonymousUserAttributes(userStruct: userStruct) : nil;
            
            return userAttributes ?? AnonymousUserAttributes();
        } catch {
            throw error;
        }
    }
    
    private static func buildAnonymousUserAttributes(userStruct: RetenoUserStruct) -> AnonymousUserAttributes {
        let fields = userStruct.userAttributes?.fields?.map { field in
            UserCustomField(key: field.key, value: field.value)
        }
